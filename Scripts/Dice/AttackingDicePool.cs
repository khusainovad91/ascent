...
public class AttackingDicePool : NetworkBehaviour
{
    ...
    [SerializeField]
    private List<AttackingDice> baseAttackingDices;

    public void EnemyAddAttackDices()
    {
        if (!IsServer)
        {
            return;
        ...
        //base.OnNetworkSpawn();
        foreach (var baseAttackDice in baseAttackingDices)
        {
            InstantiateAndAddDiceToDicePool((int)baseAttackDice, true);
        }
    ...
    public virtual void TryAddDicesByWeaponCard(WeaponItem weaponCard)
    {
        if (!IsServer)
        {
            return;
        ...
        if (currentWeapon != null && currentWeapon == weaponCard) { return; }

        if (weaponCard.attackingDicePrefabsNumbers != null && weaponCard.attackingDicePrefabsNumbers.Count > 0)
        {
            foreach (int dicePrefabNumber in weaponCard.attackingDicePrefabsNumbers)
            {
                InstantiateAndAddDiceToDicePool(dicePrefabNumber, true);
            }
            currentWeapon = weaponCard;
            HideDicesRpc(0);
            Debug.Log("Resseting attacking dice for weapon");
            ResetDicesPosition(Constants.MID_ABOVE_CHARACTER);
        }
    ...
    //[Rpc(SendTo.Everyone)]
    //private void InstantiateAndAddDiceToDicePoolClientRpc(NetworkObjectReference diceRef)
    //{
    //    if (diceRef.TryGet(out var diceNO))
    //    {
    //        diceNO.transform.SetParent(this.transform, false);
    //        //diceNO.transform.localScale = Constants.DEF_SMALL_CUBE_SIZE;
    //    }
    //    _dices.Add(diceNO.GetComponent<DiceData>());
    //}
